...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 15 13:09:58 2025
...
rh_data = rh_data.dropna(subset=['Rain_in_Tot_Aasu'])  # Keep only rows where RH_Aasu is not NaN

def feature_engineering(df):
    df = df.copy()
    df['TIMESTAMP'] = pd.to_datetime(df['TIMESTAMP'])
    target_column = 'Rain_in_Tot_Aasu'
    feature_cols = [col for col in df.columns if col not in ['TIMESTAMP', target_column,'Elevation_target','synoptic_elevation_1','synoptic_elevation_0','is_rain']]    
    ...
    df['solar_per_temp'] = df['SlrMJ_Tot_Aasu_rolling4'] / (df['PTemp_C_Max_Aasu_rolling6'] + 1e-3)
...
estimators = [
    ...
    ('lgb', LGBMRegressor(**best_params_lgbm, random_state=42)),
]
...
# --- Export error metrics for XGBoost, LightGBM, and Stacked Ensemble to CSV ---
...
y_pred_stack = y_pred_stack  # already computed above
...
for name, pred in [
    ...
    ("StackedEnsemble", y_pred_stack),
]:
    metrics.append({
        "model": name,
        "MAE": mean_absolute_error(y_test, pred),
        "R2": r2_score(y_test, pred),
        "MAPE_%": safe_mape(y_test, pred)
    })

metrics_df = pd.DataFrame(metrics)
# Save CSV (adjust path if you prefer a different location/name)
out_path = "/Users/lizamclatchy/ASCDP/Data Cleaning/Cleaned Model Input Data/aasurain_model_error_metrics.csv"
...
evaluate_predictions(y_test, model_xgb.predict(X_test), "XGBoost")
evaluate_predictions(y_test, model_lgbm.predict(X_test), "LightGBM")
evaluate_predictions(y_test, y_pred_stack, "Stacked Ensemble")

plt.figure(figsize=(14, 6))
plt.plot(y_test.index, y_test, label='Actual', color='blue', linewidth=1.5)
plt.plot(y_test.index, y_pred_stack, label='Stacked Prediction', color='purple', linestyle='--', linewidth=1.5)

plt.title("Actual vs Stacked Prediction")
plt.xlabel("Index")
plt.ylabel(target_column)
plt.legend()
plt.grid(True)
