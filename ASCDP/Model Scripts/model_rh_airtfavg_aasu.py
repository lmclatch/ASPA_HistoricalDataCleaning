...
combined_df = pd.read_csv("/Users/lizamclatchy/Documents/GitHub/ASPA_HistoricalDataCleaning/ASCDP/Data Cleaning/Cleaned Model Input Data/aasu_rh_train.csv")
selected_columns = ['TIMESTAMP', 'RH_Aasu'] + [col for col in combined_df.columns if col not in ['TIMESTAMP', 'RH_Aasu']]
rh_data = combined_df[selected_columns].copy()
rh_data = rh_data.dropna(subset=['RH_Aasu'])  # Keep only rows where is not NaN
...
out_path = "/Users/lizamclatchy/Documents/GitHub/ASPA_HistoricalDataCleaning/ASCDP/Results Analysis/RH_Aasu_error_metrics.csv"
...
import joblib
joblib.dump(model_xgb, 'RH_Aasu_model_xgb.pkl')
joblib.dump(model_lgbm, 'RH_Aasu_model_lgbm.pkl')
joblib.dump(stack, 'RH_Aasu_stack.pkl')
...
TITLE = "Relative Humidity of Aasu (%)"
