...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 16 08:53:53 2025
...
combined_df = pd.read_csv("/Users/lizamclatchy/ASCDP/Data Cleaning/Cleaned Model Input Data/vaipito_SlrW_Avg_train.csv")
...
rh_data = rh_data.dropna(subset=['SlrW_Avg_Vaipito'])  # Keep only rows where RH_Aasu is not NaN
...
def xgboost_regression(X_train, X_test, y_train, y_test, target_column, n_splits=10):
    ...
    plt.title(f'Actual vs Predicted SlrW_Avg_Vaipito({target_column})')
    plt.xlabel("Index")
    plt.ylabel("SlrW_Avg_Vaipito")
    plt.legend()
    plt.tight_layout()
    plt.show()
