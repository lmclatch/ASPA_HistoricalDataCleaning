...
cleaned_data_dir = "/Users/lizamclatchy/ASCDP/Data Cleaning/Cleaned Raw Data"

# Loop through dataframes and save cleaned versions
for name, df in dataframes.items():
    if 'ALL' in name.upper():
        # Extract station name (e.g., from "Aasu_ALL_data" -> "Aasu")
        station_match = re.match(r'([A-Za-z0-9]+)', name)
        if station_match:
            station_name = station_match.group(1)
        else:
            print(f"Could not parse station name from {name}, skipping.")
            continue

        # Create target path for the cleaned CSV
        file_path = os.path.join(cleaned_data_dir, f"{station_name}_cleaned.csv")
        ...
        try:
            ...
            print(f"❌ Error saving file for {station_name}: {e}")
